...
function main() {
    ...
    const filename = path.join(__dirname, "./claimers.csv");

    // what file should we write the merkle proofs too?
    const output_file = path.join(__dirname, "./claimers.json");
    ...
    fs.createReadStream(filename)
        ...
        .on("data", (row) => {
            ...
            const leaf_hash = utils.solidityKeccak256(
                ["address", "uint256", "uint256"], [row["user_address"], row["itemID"], row["amount"]]
